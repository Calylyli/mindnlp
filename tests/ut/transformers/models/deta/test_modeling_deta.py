...
import unittest
import numpy as np
from mindnlp.transformers import DetaConfig
from mindnlp.utils.testing_utils import is_mindspore_available, is_vision_available
    ...
    def prepare_config_and_inputs(self):
        ...
        if self.use_labels:
            ...
            for i in range(self.batch_size):
                target = {}
                target["class_labels"] = ops.randint(low=0,
                    high=self.num_labels, size=(self.n_targets,)
                ).astype(mindspore.int32)
                target["boxes"] = ops.rand(self.n_targets, 4)
                target["masks"] = ops.rand(self.n_targets, self.image_size, self.image_size)
...
class DetaModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        model = DetaForObjectDetection.from_pretrained("jozhang97/deta-resnet-50")
        ...
        self.assertTrue(np.allclose(outputs.logits[0, :3, :3], expected_logits, atol=1e-4))

        expected_shape_boxes = (1, 300, 4)
        self.assertEqual(outputs.pred_boxes.shape, expected_shape_boxes)
        self.assertTrue(np.allclose(outputs.pred_boxes[0, :3, :3], expected_boxes, atol=1e-4))
