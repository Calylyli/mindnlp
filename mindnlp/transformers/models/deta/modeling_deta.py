...
def _batched_nms_coordinate_trick(
    ...
    Args:
        ...
        iou_threshold (float): The intersection over union (IoU) threshold used for Nmindspore. Boxes with IoU higher than this threshold will be suppressed.
    Returns:
        mindspore.Tensor: A tensor containing the indices of the boxes to keep after Nmindspore. Its shape is (M,), where M is the number of boxes to keep.
    Raises:
        None.
    """
    ...
    offsets = idxs.to(boxes.dtype) * (max_coordinate + mindspore.tensor(1).to(boxes.dtype))
...
def nms(boxes: mindspore.Tensor, scores: mindspore.Tensor, iou_threshold: float):
    ...
    Args:
        ...
        iou_threshold (float): The Intersection over Union (IoU) threshold used for Nmindspore.
            Bounding boxes with IoU greater than or equal to this threshold will be suppressed.
    Returns:
        mindspore.Tensor: A tensor containing the indices of the selected bounding boxes after Nmindspore.
...
class DetaFrozenBatchNorm2d(nn.Cell):
    """
...
class DetaBackboneWithPositionalEncodings(nn.Cell):
    ...
    def construct(self, pixel_values: mindspore.Tensor, pixel_mask: mindspore.Tensor):
        ...
        for feature_map in features:
            ...
            mask = ops.interpolate(pixel_mask[None].float(), size=feature_map.shape[-2:]).to(mindspore.bool_)[0]
...
def _expand_mask(mask: mindspore.Tensor, dtype: mindspore.dtype, target_len: Optional[int] = None):
    """
    Expands attention_mask from `[batch_size, seq_len]` to `[batch_size, 1, target_seq_len, source_seq_len]`.
    """
    batch_size, source_len = mask.shape
    ...
    return inverted_mask.masked_fill(inverted_mask.bool(), np.finfo(mindspore.dtype_to_nptype(dtype)).min)
...
class DetaSinePositionEmbedding(nn.Cell):
    """
    ...
    def construct(self, pixel_values, pixel_mask):
        ...
        dim_t = ops.arange(self.embedding_dim, dtype=mindspore.float32)
...
class DetaLearnedPositionEmbedding(nn.Cell):
    """
    ...
    def construct(self, pixel_values, pixel_mask=None):
        ...
        pos = ops.cat([x_emb.unsqueeze(0).repeat(height, 1, 1), y_emb.unsqueeze(1).repeat(1, width, 1)], axis=-1)
...
class DetaMultiscaleDeformableAttention(nn.Cell):
    """
...
class DetaMultiheadAttention(nn.Cell):
    """
...
class DetaEncoderLayer(nn.Cell):
...
class DetaDecoderLayer(nn.Cell):
    def __init__(self, config: DetaConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
class DetaClassificationHead(nn.Cell):
...
class DetaEncoder(DetaPreTrainedModel):
    ...
    def __init__(self, config: DetaConfig):
        ...
        self.dropout = config.dropout
        self.layers = nn.CellList([DetaEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class DetaModel(DetaPreTrainedModel):
    def __init__(self, config: DetaConfig):
        ...
        self.level_embed = Parameter(ops.zeros([config.num_feature_levels, config.d_model]))

        if config.two_stage:
            self.enc_output = nn.Dense(config.d_model, config.d_model)
            self.enc_output_norm = nn.LayerNorm(config.d_model)
            self.pos_trans = nn.Dense(config.d_model * 2, config.d_model * 2)
            self.pos_trans_norm = nn.LayerNorm(config.d_model * 2)
            self.pix_trans = nn.Dense(config.d_model, config.d_model)
            self.pix_trans_norm = nn.LayerNorm(config.d_model)
        else:
            self.reference_points = nn.Dense(config.d_model, 2)
    ...
    def get_proposal_pos_embed(self, proposals):
        ...
        dim_t = ops.arange(num_pos_feats, dtype=mindspore.float32)
        ...
        pos = ops.stack((pos[:, :, :, 0::2].sin(), pos[:, :, :, 1::2].cos()), axis=4).flatten(2)
        return pos
    ...
    def construct(
        ...
        if self.config.num_feature_levels > len(sources):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                mask = ops.interpolate(pixel_mask[None].float(), size=source.shape[-2:]).to(mindspore.bool_)[0]
            ...
            if self.assign_first_stage:
                ...
                for b in range(batch_size):
                    ...
                    if len(keep_inds) < self.two_stage_num_proposals:
                        ...
                        keep_inds = ops.topk(proposal_logit[b], topk)[1]
                ...
                topk_proposals = ops.stack(topk_proposals)
            else:
...
class DetaLoss(nn.Cell):
    ...
    def construct(self, outputs, targets):
        ...
        num_boxes = ops.clamp(num_boxes, min=1).item()
...
class DetaMLPPredictionHead(nn.Cell):
    """
...
class DetaHungarianMatcher(nn.Cell):
    """
    ...
    def construct(self, outputs, targets):
        ...
        target_bbox = ops.cat([v["boxes"] for v in targets])
...
def box_iou(boxes1, boxes2):
    ...
    right_bottom = ops.maximum(boxes1[:, None, 2:], boxes2[:, 2:])  # [N,M,2]
...
def generalized_box_iou(boxes1, boxes2):
    ...
    top_left = ops.maximum(boxes1[:, None, :2], boxes2[:, :2])
...
def nonzero_tuple(x):
    if x.dim() == 0:
        out_x = ops.nonzero(x.unsqueeze(0))
        return out_x.unbind(1)
    else:
        return ops.nonzero(x).unbind(1)
...
class DetaMatcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        assert ops.all(match_quality_matrix >= 0)

        # match_quality_matrix is M (gt) x N (predicted)
        # Max over gt elements (dim 0) to find best gt candidate for each prediction
        matched_vals, matches = match_quality_matrix.max(dim=0)

        match_labels = matches.new_full(matches.shape, 1, dtype=mindspore.int8)
...
class DetaStage2Assigner(nn.Cell):
    ...
    def _sample_proposals(self, matched_idxs: mindspore.Tensor, matched_labels: mindspore.Tensor, gt_classes: mindspore.Tensor):
        ...
        if has_gt:
            ...
            gt_classes = ops.zeros_like(matched_idxs) + self.bg_label
    ...
    def construct(self, outputs, targets, return_cost_matrix=False):
...
class DetaStage1Assigner(nn.Cell):
